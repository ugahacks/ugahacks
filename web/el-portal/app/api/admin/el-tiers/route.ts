...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = adminDb.collection('el-tiers');
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingTierQuery = await adminDb.collection('el-tiers').where('userId', '==', userId).get();
    ...
    if (!existingTierQuery.empty) {
      ...
      await adminDb.collection('el-tiers').doc(tierId).update({
        ...tierData,
        createdAt: existingTier.data().createdAt // Preserve original creation date
      });
    } else {
      // Create new tier assignment
      const tierRef = await adminDb.collection('el-tiers').add(tierData);
      tierId = tierRef.id;
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await adminDb.collection('el-tiers').doc(tierId).delete();
